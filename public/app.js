(function(){
  ...
  async function openNotesEditor(noteId, position){
    const existing = noteId ? (state.notes||[]).find(n=>n.id===noteId) : null;
    let telegramRecipients=[];
    try{
      const suffix=window.activeWorkspace?`?canvas=${encodeURIComponent(window.activeWorkspace)}`:'';
      const response=await fetch('/api/telegram-recipients'+suffix,{credentials:'include'});
      if(response.ok) telegramRecipients=(await response.json()).recipients||[];
    }catch(e){ /* Reminder still works without an explicit tag. */ }
    ...
    const syncReminderInputs=()=>{reminderDate.disabled=!reminderCheck.checked;reminderTime.disabled=!reminderCheck.checked;reminderRecipient.disabled=!reminderCheck.checked;}; reminderCheck.addEventListener('change',syncReminderInputs); syncReminderInputs();
    ...
    const save=document.createElement('button'); save.className='btn btn-fan'; save.type='button'; save.textContent='Сохранить'; save.addEventListener('click',()=>{
      ...
      if(existing){
        ...
        if(reminderCheck.checked){if(existing.reminderAt!==nextReminderAt||existing.reminderRecipientId!==reminderRecipient.value){delete existing.reminderSentFor;delete existing.reminderSentAt;}existing.reminderAt=nextReminderAt;existing.reminderRecipientId=reminderRecipient.value;}
        else {delete existing.reminderAt;delete existing.reminderRecipientId;delete existing.reminderSentFor;delete existing.reminderSentAt;}
      }
      else { const p=position||{x:160,y:120}; state.notes.push({id:uid(),title:title.value.trim(),text:text.value.trim(),x:Math.max(0,Math.round(p.x-120)),y:Math.max(0,Math.round(p.y-58)),createdAt:Date.now(),reminderEnabled:reminderCheck.checked,...(reminderCheck.checked?{reminderAt:nextReminderAt,reminderRecipientId:reminderRecipient.value}:{})}); }
  ...
  function renderLaunchPlanView(){
    ...
    plans.forEach(plan=>{const creative=(plan.creativeName||'Без нейма').trim()||'Без нейма';const list=groups.get(creative)||[];list.push(plan);groups.set(creative,list);});
    wrap.innerHTML=[...groups.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([geo,items])=>{
      ...
      return `<details class="launch-geo-group" open><summary class="launch-geo-summary"><span class="launch-geo-label">КРЕО</span><strong class="launch-creative-name">${escapeHtml(geo)}</strong><span class="launch-geo-count">${items.length} камп. · ${quantity} шт.</span><button class="accs-mini-btn add-launch-geo" type="button" data-creative-name="${escapeHtml(geo)}">+ GEO</button></summary><table class="launch-plan-table"><thead><tr><th>GEO</th><th>Кол-во</th><th>Сетап кампании</th><th>Бюджет</th><th></th></tr></thead><tbody>${items.map(plan=>`<tr data-launch-plan-id="${plan.id}"><td><span class="launch-geo-name">${escapeHtml(plan.geo||'—')}</span></td><td>${escapeHtml(plan.quantity||'—')}</td><td>${escapeHtml(plan.campaignSetup||'—')}</td><td>${escapeHtml(plan.budget||'—')}</td><td class="row-actions"><button class="btn-plain edit-launch-plan" type="button">Изм.</button></td></tr>`).join('')}</tbody></table></details>`;
    }).join('');
    ...
    wrap.querySelectorAll('.add-launch-geo').forEach(button=>button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();openLaunchPlanEditor(null,button.dataset.creativeName);}));
  }
  function openLaunchPlanEditor(id, presetCreativeName=''){
    ...
    const fields=[
      ...
      ['geo','GEO · можно несколько через запятую','Например: EB, EA, ER','text'],
    ...
    const inputs={};
    fields.forEach(([key,label,placeholder,type])=>{const field=document.createElement('div');field.className='field';field.innerHTML=`<label>${label}</label>`;const input=document.createElement('input');input.type=type;input.placeholder=placeholder;input.value=existing?(existing[key]||''):(key==='creativeName'?presetCreativeName:'');if(type==='number')input.min='0';if(key==='campaignSetup'){const listId='launchSetupOptions';let list=document.getElementById(listId);if(!list){list=document.createElement('datalist');list.id=listId;document.body.appendChild(list);}const setups=Array.from(new Map((state.launchPlans||[]).map(plan=>(plan.campaignSetup||'').trim()).filter(Boolean).map(setup=>[setup.toLocaleLowerCase(),setup])).values()).sort((a,b)=>a.localeCompare(b));list.innerHTML=setups.map(setup=>`<option value="${escapeHtml(setup)}"></option>`).join('');input.setAttribute('list',listId);field.querySelector('label').textContent='Сетап кампании · выбери сохранённый или введи новый';}field.appendChild(input);modal.appendChild(field);inputs[key]=input;});
    const actions=document.createElement('div');actions.className='modal-actions';
  ...
  function reminderCountdown(task){
    ...
    if(taskIsDailyDone(task)) return `Следующее завтра: ${formatted}`;
  ...
  function renderTasksView(){
    ...
    const activeTasks = tasks.filter(t=>!t.dailyReminder && !taskIsArchived(t)).filter(matches);
    const shown = filter === 'archive' ? [] : activeTasks;
    renderDailyTasks(tasks, matches);
  ...
  function setupTasksDelegation(){
    const wrap = document.getElementById('tasksBoardWrap');
    const dailyWrap = document.getElementById('dailyTasksWrap');
    wrap.addEventListener('click', safe((e)=>{
