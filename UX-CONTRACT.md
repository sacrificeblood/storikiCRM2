...
- «Accs» stores each agent and soc disclosure state locally for the current browser. Agent order is a saved workspace property and can be changed with the visible up/down controls. An account marked as banned (or with a ban date) can be hidden without deletion; the «Показать скрытые забаненные» filter reveals it and the restore action returns it to the normal list.
