...
- Access is authenticated by a secure server session. Admins create buyers; buyers create assistants in their own workspace. Buyers can create regular tasks, move and confirm existing task cards, and start the five-minute timer for daily tasks in their assigned CRM. Starting that timer explicitly reactivates a completed daily task so the reminder worker does not skip it. Daily-task creation and reminder settings remain administrator-owned. Assistants receive permission-scoped shared access and can always confirm daily tasks; the server preserves task content and enforces the same transition policy for every API request.
...
- The CRM relationship map and all canvas ownership/access controls are administrator-only. Buyers and assistants use their assigned CRM directly and cannot open the canvas-management route or mutate canvas relationships through the API.
- «Уникализатор» is available to authenticated users and processes one source image entirely in the browser. The person chooses 1–30 variants, sees determinate per-variant progress, can download each result or request all completed results, and keeps generated object URLs only for the current browser session. The workflow strips source metadata through canvas re-encoding and reports unsupported browser image formats without uploading the source.
